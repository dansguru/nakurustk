...
export const findBookingByCheckoutRequestID = async (
    checkoutRequestID: string
): Promise<{ success: boolean; bookingId?: string; error?: any }> => {
    try {
        if (!db) {
            console.error("Firebase Database not initialized");
            return { success: false, error: "Firebase Database not initialized" };
        ...
        // Look for the booking that has this checkoutRequestID
        const bookingsRef = db.ref('bookings');
        const snapshot = await bookingsRef.orderByChild('checkoutRequestID').equalTo(checkoutRequestID).get();
        ...
        if (snapshot.exists()) {
            ...
            if (bookingId) {
                console.log("🔍 Found booking for checkoutRequestID:", { checkoutRequestID, bookingId });
        ...
        console.error("❌ Error finding booking by checkoutRequestID:", error);
        return { success: false, error };
    }
};
